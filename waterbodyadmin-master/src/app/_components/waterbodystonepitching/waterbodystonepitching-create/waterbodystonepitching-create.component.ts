...
@Component({
  selector: 'app-waterbodystonepitching-create',
  templateUrl: './waterbodystonepitching-create.component.html',
  styleUrls: ['./waterbodystonepitching-create.component.css']
})
export class WaterbodystonepitchingCreateComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private waterbodystonepitchingService: WaterbodystonepitchingService,
  ...
  ngOnInit(): void {
    ...
    if (!this.isAddMode) {
      this.waterbodystonepitchingService.get(this.id)
        .pipe(first())
  ...
  private createResource() {
    ...
    this.waterbodystonepitchingService.create(newresource)
      .pipe(first())
      .subscribe({
        next: () => {
  ...
  private updateResource() {
    ...
    this.waterbodystonepitchingService.update(this.id, updatedresource)
      .pipe(first())
      .subscribe({
        next: () => {
          let dialogRef = this.dialog.open(SuccessDialogComponent, this.dialogConfig);
