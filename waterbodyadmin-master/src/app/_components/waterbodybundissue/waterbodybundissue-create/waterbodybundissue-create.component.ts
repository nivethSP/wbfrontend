...
@Component({
  selector: 'app-waterbodybundissue-create',
  templateUrl: './waterbodybundissue-create.component.html',
  styleUrls: ['./waterbodybundissue-create.component.css']
})
export class WaterbodybundissueCreateComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private waterbodybundissuesService: WaterbodybundissuesService,
  ...
  ngOnInit(): void {
    ...
    if (!this.isAddMode) {
      this.waterbodybundissuesService.get(this.id)
        .pipe(first())
  ...
  private createResource() {
    ...
    this.waterbodybundissuesService.create(newresource)
      .pipe(first())
      .subscribe({
        next: () => {
  ...
  private updateResource() {
    ...
    this.waterbodybundissuesService.update(this.id, updatedresource)
      .pipe(first())
      .subscribe({
        next: () => {
          let dialogRef = this.dialog.open(SuccessDialogComponent, this.dialogConfig);
