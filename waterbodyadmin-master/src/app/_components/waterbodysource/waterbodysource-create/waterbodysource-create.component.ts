...
@Component({
  selector: 'app-waterbodysource-create',
  templateUrl: './waterbodysource-create.component.html',
  styleUrls: ['./waterbodysource-create.component.css']
})
export class WaterbodysourceCreateComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private waterbodysourceService: WaterbodysourceService,
  ...
  ngOnInit(): void {
    ...
    if (!this.isAddMode) {
      this.waterbodysourceService.get(this.id)
        .pipe(first())
  ...
  private createResource() {
    ...
    this.waterbodysourceService.create(newresource)
      .pipe(first())
      .subscribe({
        next: () => {
  ...
  private updateResource() {
    ...
    this.waterbodysourceService.update(this.id, updatedresource)
      .pipe(first())
      .subscribe({
        next: () => {
          let dialogRef = this.dialog.open(SuccessDialogComponent, this.dialogConfig);
